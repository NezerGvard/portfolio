...
async def on_message(ctx):
    if ctx.author.bot == False:
        ...
        response = requests.post(
            ...
            files={'image_file': open(r"C:\Users\Denis\Desktop\python\discord\image\remove_bg\u.png", 'rb')},
            data={'size': 'auto'},
            headers={'X-Api-Key': 'your-api'},
            
        )
        if response.status_code == requests.codes.ok:
            with open(r'C:\Users\Denis\Desktop\python\discord\image\remove_bg\rb.png', 'wb') as out:
                out.write(response.content)
        else:
            print("Error:", response.status_code, response.text)

        file = discord.File(r"C:\Users\Denis\Desktop\python\discord\image\remove_bg\rb.png")
